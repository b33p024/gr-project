...
def print_tls_flows(tls_dict,total_packets):
    ...
    for key,value in tls_dict.items():
        ...
        print("Host " + key+ " generated "+str(pkt_count)+"("+str(p)+"% of the total) packets.")
        #******
        #Analyze TLS traffic
        ...
        #put the unknown flows at the end of the list
        ...
        for flow in sorted_flows:
            ...
            if socket.gethostbyname(flow.requested_server_name) == flow.dst_ip :
                ...
                print("                   "+ flow.application_name + "  --  "+flow.src_ip+"  --  "+flow.dst_ip+ "  --  "+flow.requested_server_name+ " ("+str(perc)+"%)  --  WARNING:SNI not authenticated" )
    ...
    for key,value in dic.items():
        ...
        for flow in value:
            ...
            if stripped in tmp:
                ...
                tmp[stripped]=0
                tmp[stripped]+=flow.bidirectional_packets
        print("Host -> "+key+" generated "+str(pkt_count)+"("+str(perc)+"%) packets:" )
        for key1,value1 in tmp.items():
            perc1=round((value1*100)/pkt_count,2)
            print("              Protocol -> "+key1+" -> "+str(value1)+"("+str(perc1)+"%) packets.")
    return tls_dict
